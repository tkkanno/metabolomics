...
def find_new_peak(ppm, data, metabolite, point, range_to_search = 0.002):
  ...
  try:
    ...
    print "something went wrong. Onto the next one:"
    next
  return new_ppm
...
return new_ppms
def find_new_peaks(ppm, data, mets, range_to_search = 0.002):
  mets.columns = ['metabolite', 'ppm']
  for i in range(len(mets)):
    idx 	= np.argmin(np.abs(ppm - float(mets.ix[i].ppm)))
    old_idx = idx
    idxmin = np.argmin(np.abs(ppm - float(mets.ix[i].ppm) - range_to_search)) #signs reversed b/c nmr spectra x-axis is reversed
    idxmax = np.argmin(np.abs(ppm - float(mets.ix[i].ppm) + range_to_search))
    sppm = ppm[idxmin:idxmax]
    max_spect = data[:, idxmin:idxmax].mean(axis = 0)
    try:
      if max_spect.max() >= data[:,idx].mean(axis = 0).max():
